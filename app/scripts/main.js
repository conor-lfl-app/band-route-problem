...
const mapDistances = (point, destinations) => {
...
const findShortestDestination = (point, destinations) => {
  const remainingDestinations = [...destinations]
  const distances = mapDistances(point, remainingDestinations)
...
const createOptimalRouteFrom = ({startingPoint, destinations, stops=[]}) => {
  // get the shortest distance from the starting point
  const finalStops = [...stops]
  const routeBuilder = ({startingPoint, destinations}) => {
    const {
      shortestDestination,
      remainingDestinations } = findShortestDestination(startingPoint, destinations)
...
const startingPoint = points40[0]
const destinations = [...points40].splice(1, points40.length - 1)
const stops = createOptimalRouteFrom({startingPoint, destinations, stops: [startingPoint]})
